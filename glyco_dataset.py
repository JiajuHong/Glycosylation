#!/usr/bin/env python
"""Dataset and validation utilities for glycosylation modeling.

This module intentionally contains no model code. It fixes the first-stage data
pipeline: reading ``glyco_model_local.csv``, parsing local atom indices and
roles, validating them against RDKit heavy-atom molecules, and producing a
batch object that downstream models can consume directly.
"""
...
from chiral_graph import CHIRAL_GRAPH_FEATURE_VERSION, mol_to_chiral_pyg_graph
    ...
    def load_graph_cache(
        ...
        if payload.get("feature_version") != expected_version:
            raise ValueError(
                ...
                f"expected {expected_version}. Rebuild it with build_rdkit_graph_cache.py."
            )
